...
export function Team() {
  ...
  const fetchTeam = async () => {
    try {
      const res = await fetch('/api/client/team', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } })
      if (res.ok) setTeam(await res.json())
      const gRes = await fetch('/api/client/team-groups', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } })
